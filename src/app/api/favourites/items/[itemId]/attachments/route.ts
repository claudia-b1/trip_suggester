...
import { verifyFavouriteItemOwnership } from "@/lib/ownership";
...
export async function POST(
  ...
  if (!await verifyFavouriteItemOwnership(itemIdNum, userId)) {
  ...
  const count = await prisma.attachment.count({ where: { favouriteItemId: itemIdNum } });
