...
import { DestinationAdvisor } from "@/components/destination-advisor";
...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-8">
      {/* Hero — only shown when user has no trips yet */}
      {!hasTrips && (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[hsl(var(--primary))]/10 via-[hsl(var(--primary))]/5 to-transparent border border-[hsl(var(--border))] px-8 py-12">
          <div className="absolute -right-8 -top-8 text-[120px] opacity-10 select-none">🌍</div>
          <div className="relative">
          ...
          <>
            <div className="mb-4 flex items-center justify-between">
              ...
              <p className="text-xs text-[hsl(var(--muted-foreground))]">
                ...
                {serializedTrips.filter((t) => !t.archived).length} trip{serializedTrips.filter((t) => !t.archived).length === 1 ? "" : "s"} planned
              </p>
            </div>
      ...
      {/* Destination advisor */}
      <div>
        <h2 className="mb-4 text-xl font-semibold">Not sure where to go?</h2>
        <DestinationAdvisor />
      </div>
    </div>
  );
